...
@Composable
fun CreateClassroomDialog(onDismiss: () -> Unit, onConfirm: (name:String, colorString:String, description:String) -> Unit){
        ...
        Surface(modifier = Modifier.fillMaxSize().background(color = OffWhite).padding(UiVariables.ScreenPadding)) {
            Column(modifier = Modifier.fillMaxWidth().background(color = OffWhite), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Top) {
                Row(modifier = Modifier.fillMaxWidth().padding(bottom = 32.dp), verticalAlignment = Alignment.CenterVertically) {
                    ...
                    Heading4("Create Classroom")
                    Spacer(modifier = Modifier.weight(1f))
                    Spacer(modifier = Modifier.width(24.dp))
                }
                FormColumn {
                    FormTextField(value = name, label = "Classroom Name", onValueChange = {name = it})
                    FormColorField(color = colorString.toColor(), label = "Classroom Color", onValueChange = {colorString = it})
                    FormTextArea(value = description, label = "Description", onValueChange = {description = it})
                }
                Spacer(modifier = Modifier.fillMaxWidth().height(24.dp))
                DefaultButton(text = "Done", onClick = {onConfirm(name, colorString, description)}, enabled = (
                        name.isNotBlank())
                )
            }
        }
